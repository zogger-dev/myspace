...
- `git/engine.rs` — all git work, via libgit2 only (**never shell out to git**). Auth: libgit2 does not read `~/.ssh/config`, so it sees neither `IdentityFile` nor `IdentityAgent`. `ssh_candidates` walks one credential per callback invocation (libgit2 re-invokes after each rejection) — an explicit per-host `ssh_key` wins outright, else the `$SSH_AUTH_SOCK` agent, then conventional `~/.ssh/id_*` files, then an error that names the fix (reading `IdentityAgent` from `~/.ssh/config` purely for that message). A configured `ssh_agent` socket is applied by pointing `SSH_AUTH_SOCK` at it during startup in `main`, before any threads exist. Worktree quirks live here: creation detaches via a scratch-branch dance (libgit2 has no detach flag), registrations are name-deduped (`free_worktree_name`) and stale ones pruned, removal is `prune` with `valid+working_tree` flags plus a manual dirty check. `owning_bare_repo` maps a worktree back to its bare repo via `commondir` — never re-derive cache paths from URLs.
...
`git2` **must** carry `features = ["ssh"]`: git2 0.21 ships `default = []`, so a plain dependency builds libgit2 with no SSH transport and every `git@host:…` clone dies with "unsupported URL protocol; class=Net". No integration test catches this on its own (they clone from local paths, which use the always-present `local` transport), so `libgit2_has_ssh_transport_compiled_in` guards it explicitly. `https` is deliberately not enabled — SSH is the only supported transport, and enabling it pulls in `openssl-sys`.
